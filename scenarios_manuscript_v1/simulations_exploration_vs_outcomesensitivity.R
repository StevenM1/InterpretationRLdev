...
simulate_fit_recover1 <- function(beta, nReps, nTrials, pReward) {
  ...
  for(ii in 1:nReps) {
    ...
    out1 <- DEoptim(ll_func, lower=c(0,0), upper=c(1,10), pnames=c('alpha', 'beta'), rewards=out$rewards, choices=out$choices, nTrials=nTrials, constants=c('gamma'=1))
...
for(figtype in c('pdf', 'png')) {
  ...
  arrows(x0=1, x1=5, y0=0.5, y1=1.5, xpd=TRUE, length=0.05,col=2,lwd=2, code=1)
  text(x=5.2, y=1.6, labels=expression(paste('Assumed ', gamma)), xpd=TRUE, col=2, pos=4)  
...
#if(figtype == 'pdf') pdf(file='./figures/scena#rio6.pdf', width=7, height=2.5)
#if(figtype == 'png') png(file='./figures/scenario6.png', width=7, height=2.5, units='in', res=175)
